 ...
 */
  ...
  #if !IS_CARTESIAN || ENABLED(X_DUAL_ENDSTOPS) || ENABLED(Y_DUAL_ENDSTOPS) || ENABLED(Z_DUAL_ENDSTOPS) || ENABLED(DUAL_X_CARRIAGE) || ENABLED(DUAL_Y_CARRIAGE) || ENABLED(X_DUAL_STEPPER_DRIVERS) || ENABLED(Y_DUAL_STEPPER_DRIVERS) || ENABLED(Z_DUAL_STEPPER_DRIVERS) || ENABLED(Z_TRIPLE_STEPPER_DRIVERS)
    #error "E_AXIS_HOMING currently requires CARTESIAN setup with 1 endstop per axis. Untested for multiple carriages or multiple stepperdrivers per axis."
